...
export type FileLoadErrorType = 'not.' | 'could.not.extract';
...
export class DataUtils {
   ...
   * @param is boolean representing if the file represented by the event is an
   ...
   */
  loadFileToForm(event: Event, editForm: FormGroup, field: string, is: boolean): Observable<void> {
    return new Observable((observer: Observer<void>) => {
      const eventTarget: HTMLInputElement | null = event.target as HTMLInputElement | null;
      if (eventTarget?.files?.[0]) {
        const file: File = eventTarget.files[0];
        if (is && !file.type.startsWith('/')) {
          const error: FileLoadError = {
            message: `File was expected to be an  but was found to be '${file.type}'`,
            key: 'not.',
